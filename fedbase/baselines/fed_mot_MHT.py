...
def run(dataset_splited, batch_size, num_nodes, model, objective, optimizer, global_rounds, local_steps, n_ensemble, H, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'),\
        weight_type='loss'):
        ...
        for n_en in range(n_ensemble):
            for h in range(H):
                ...
                for j in range(num_nodes):
                    ...
                    nodes[j].local_update_steps(local_steps, partial(nodes[j].train_single_step_bayes, reg_model = cluster_models[h][n_en], reg_model_lambda = cluster_models_lambda[h][n_en]))
