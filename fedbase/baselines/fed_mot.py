...
def run(dataset_splited, batch_size, K, num_nodes, model, objective, optimizer, global_rounds, local_steps, \
    ...
    for t in range(global_rounds):
        ...
        if reduction == 'JPDA':
            ...
            # get the biggest value of nodes_k_m_weight (min value of loss)of each node and its index
            nodes_k_m_weight_max = [min(ce_loss_k_m[j]) for j in range(num_nodes)]
            nodes_k_m_weight_max_index = [ce_loss_k_m[j].index(min(ce_loss_k_m[j])) for j in range(num_nodes)]
            ...
            for i in range(K):
                ...
                if len(assign_ls) != 0:
                    ...
                    model_k = cluster_models[i].state_dict()
                    model_k_lambda = cluster_models_lambda[i]
